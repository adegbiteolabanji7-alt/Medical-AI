...
data_loader.py
--------------
MONAI data pipeline for 3D medical image segmentation.

Loads NIfTI volumes, applies preprocessing transforms,
and creates PyTorch DataLoaders for training and validation.

Why synthetic data for development?
Real medical volumes are 100MB to 500MB each. Synthetic data
lets you verify pipeline logic without large downloads.
Swap dataset path for real data when available - everything
else stays identical.

Why separate train and val transforms?
Training transforms include augmentation - random flips and
rotations that create new examples from existing data.
Validation transforms do not augment - we need consistent
evaluation to measure true model performance.
"""
...
def create_synthetic_volume(shape=(64, 64, 32)):
    """
    Create synthetic CT-like volume and segmentation mask.

    Volume simulates soft tissue HU values.
    Mask simulates a single organ as a sphere.
